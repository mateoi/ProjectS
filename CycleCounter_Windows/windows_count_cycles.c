...
*/
void calculate_mul_time(int number_of_iterations, uint64_t* array) {
...
/*
* Calculates the clock cycles taken to perform a 64-bit addition
* for the given number of times, and stores the results in the given array,
* which must be of (at least) the required size.
*/
void calculate_add_time(int number_of_iterations, uint64_t* array) {
    uint32_t time0, time1;
...
int main(int argc, char** argv) {
    ...
    if (argc >= 3 && strcmp(argv[2], "add")) {
        calculate_add_time(iterations, values);
    } else {
        calculate_mul_time(iterations, values);
    }
